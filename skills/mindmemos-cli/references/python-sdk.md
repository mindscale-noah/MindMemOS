...
from `mindmemos auth` → empty (calls then raise `AuthRequiredError`). A configured
default `user_id` is inherited by operations that require a user, such as add,
but not by search. Search only uses its per-call `user_id`; omitting it selects
project-wide search.
...
`agent_id`, `session_id`) override the client defaults. Search is the exception:
its `user_id` is per-call only and does not inherit the client default. Omit it
for project-wide search. Request IDs are generated by the server and returned in
SDK results and API errors; callers cannot provide them.
...
async def main():
    async with AsyncHttpTransport(
        ...
        result = await memory.search("seating preference", top_k=3, user_id="alice")
