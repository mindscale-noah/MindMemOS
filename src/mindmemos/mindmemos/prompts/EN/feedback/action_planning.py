...
- scenario_specific memories must include explicit scenario information in after_content — state when or under what conditions the memory applies (e.g. "When working on...", "For projects that..."). An action derived from a scenario_specific signal without an explicit scenario precondition in after_content is invalid.
- long_term memories must not include scenario preconditions, task-type qualifiers, or project-specific context in after_content. Write them as general, universally applicable statements. An action derived from a long_term signal that narrows the memory to a scenario is invalid.
