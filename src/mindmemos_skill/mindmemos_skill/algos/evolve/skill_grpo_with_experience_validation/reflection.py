"""Reflexion feedback between consecutive attempts of one task."""
    ...
    async def reflect(self, trajectory: Trajectory, *, sample_index: int) -> str:
        ...
        async def call() -> str:
            return await chat_content(
                self._chat_model,
                task=f"skill_grpo_experience_validation.reflection.{trajectory.task.task_id}.{sample_index}",
                messages=messages,
            )
...
def task_with_reflection(task: Task, *, answer: str, reflection: str) -> Task:
    context = (
    ...
    return task.model_copy(
        update={"instruction": f"{task.instruction.rstrip()}\n\n{context}", "metadata": metadata},
        deep=True,
    )
