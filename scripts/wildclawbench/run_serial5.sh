...
for category_dir in $categories; do
  for task_file in "$category_dir"/*task_*.md; do
    ...
    # Resume support: set RESUME_AFTER=YYYYMMDD_HHMM to skip every task that
    # already has a scored run dir stamped after that time (lexicographic
    # compare against the run-dir name works across midnight). A killed sweep
    # continues where it stopped instead of re-adding duplicate memories;
    # failed/no-score tasks are re-run automatically.
    if [[ -n "${RESUME_AFTER:-}" ]]; then
      ...
      if [[ $skip -eq 1 ]]; then
        echo "=== skipping $rel_task (scored run exists after $RESUME_AFTER) ==="
        continue
      fi
    fi
